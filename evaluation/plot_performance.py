...
# Save the most time-consuming tests to a file
# print("\nWriting detailed test statistics to 'test_statistics.txt'...")
# with open('test_statistics.txt', 'w') as f:
#     f.write("Top 20 Most Time-Consuming Tests:\n")
#     f.write("-" * 50 + "\n")
#     top_20 = df.sort_values('Simulated Cycles', ascending=False).head(20)
#     for idx, row in top_20.iterrows():
#         f.write(f"\nTest: {row['Test Path']}\n")
#         f.write(f"Simulated Cycles: {row['Simulated Cycles']:,.0f}\n")
#         f.write(f"Efficiency: {row['Efficiency']:.2%}\n")
#         f.write(f"Busy Cycles: {row['Cycles Busy']:,.0f}\n")
#         f.write(f"Idle Cycles: {row['Cycles Idle']:,.0f}\n")
#         f.write("-" * 30 + "\n") 